...
export function useLayeredMouseParallax(
  layers: { id: string; strength: number }[],
  config: Omit<MouseParallaxConfig, 'strength'> = {}
): Map<string, MouseParallaxResult> {
  const { springConfig = DEFAULT_SPRING, disabled = false } = config
  ...
  const result = new Map<string, MouseParallaxResult>()

  for (const layer of layers) {
    const offsetX = useTransform(mouseX, (v) => v * layer.strength)
    const offsetY = useTransform(mouseY, (v) => v * layer.strength)
    const x = useSpring(offsetX, springConfig)
    const y = useSpring(offsetY, springConfig)
    result.set(layer.id, { x, y })
  }
